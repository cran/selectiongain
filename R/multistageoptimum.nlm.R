...
`multistageoptimum.nlm`<-function(corr, ini.value=c(c(length(N.upper):1)+Budget/(CostC+sum(CostTv)+1)), Budget, CostC=1, CostTv=rep(1,length(N.upper)), Nf=1, iterlim=100, alg=GenzBretz(),N.upper, N.lower=rep(N.fs,length(N.upper)))
